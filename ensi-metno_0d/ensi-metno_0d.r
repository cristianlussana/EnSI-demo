...
#
#obs_pert <- function( ii, obs, ws, coeff, fun) {
#  if (is.na(obs[ii])) return(NA)
#  if ( (ii-ws) < 1 | length(obs) < (ii+ws)) return(obs[ii])
#  if (fun =="max") ref <- max(obs[(ii-ws):(ii+ws)], na.rm=T)
#  if (fun =="min") ref <- min(obs[(ii-ws):(ii+ws)], na.rm=T)
#  return((1-coeff) * obs[ii] + coeff * ref)
#}
...
if (argv$skip) {
  ...
  Ebr_saved <- Ebr
  Ebl_saved <- Ebl
  save(file=argv$ffout, envtmp_saved, El_saved, Er_saved, Ea_saved, yo_saved, argv_saved, in2d_obs_val, in2d_grid_val, in2d_grid_y, in2d_grid_x, in2d_grid_z, in2d_obs_y, in2d_obs_x, in2d_obs_z, Ebr_saved, Ebl_saved)
  q()
}
#------------------------------------------------------------------------------
# -- EnSI --
envtmp$HEr <- envtmp$yo; envtmp$HEr[] <- NA
for (e in 1:envtmp$k_dim) envtmp$HEr[,e] <- envtmp$Er[,e]
...
HEbr_mean <- rowMeans(Ebr)
HEbr_sd   <- apply( Ebr, FUN=function(x){sd(x)}, MAR=1)
...
envtmp$Zr[,] <- 1/sqrt(envtmp$k_dim-1) * (Ebr - HEbr_mean) / HEbr_sd
